...
**Étapas del modelado:** Antariormente se habló de los distintos tipos de aprendizaje, sin embargo, cada aprendizaje debe seguir una estructura de modelado en forma general; dicha estructura está compuesta por los siguientes pasos:

- Preprocesamiento: Consiste en dar forma a los datos con base en el model a usar. Por ejemplo, algunos modelos requieren datos estandarizado o que estén en un mismo rango; o a su vez, los datos presentan variables irrelevantes o que están correlacionadas con otras y por ende su aporte se puede ver en función de otra variable (tambíen si se quiere eliminar el ruido que generan algunos variables), en tal caso se usarían las técnicas de reducción de dimensiones antes del modelado. La etapa de preprocesamiento es fundamental, ya que esta depura y/o prepara el alimento del modelo.
...
<img title="" src="file:///home/juan-c/snap/marktext/9/.config/marktext/images/2022-11-10-20-01-49-image.png" alt="" data-align="center" width="390">

### Aprendizaje del perceptrón

Ahora bien, el trabajo de aprendizaje es básicamente la forma en que aprende el  goritmo con base en los datos, es decir, la forma de estimación del vector de parámetros o pesos $W$, pues estos indican en mayor medida el grado de activación de la neurona.  Los pasos del perceptrón inicial de Rosenblatt son:
...
EL cambio en el parámetro vienen dado por el criterio de la función de error o costo. Dado que se quiere que el modelo aprenda o autoestime los parametros, este deberá estimarlos variando cada $w_j$ tal que este haga mínimo el error entre la predicción y su valor real, es decir:

                                                          $L= \frac{1}{n} \sum_{i=1}^n(y_i-\phi(W^Tx_{ij}+b))^2$

                                                          $min(L):\frac{\partial L}{\partial w}=0$

- Donde $L$ es la función de error.

Dicho mínimo se encuentra con el criterio de la  derivada parcial (En los casos más sencillos, la función de pérdida es conveza, es decir, tiene un mínimo y es fácil de encontrar cuando el gradiente es 0). El cambio de $w_j$ comple con la condición $\frac{\partial L}{\partial w}=0$, es decir, que minimiza la funcíon de pérdida, es: 

                        $\Delta w_j=-\frac{\partial L}{\partial w}=\sum_{i=1}^n W^Tx_{ij}-y_i=\eta(y_i-\hat{y_i})x_{ij}$

Donde:

- $\eta: \in(0,1] $ : Es una constante , denominada como ratio o tasa de aprendizaje. Hiperparámetro que para las redes multicapas, controla la velocidad o el paso con el que aprende el algoritmo (si la tasa es muy baja da paso muy cortos y este no puede ser óptimo en tiempoy, si por otro lado es muy alta, da pasos muy alrgos y puede perderse en la estimación).

- $y_i:$ Es la etiqueta, en función de 0 o 1 (es decir, la etiqueta real ya clasificada con base en la función de activación $\phi(Z)$) .

- $x_{ij}$: Definido de forma vectorial es el patrón 

**Note la utilidad de la función de actualziación de los parámetro $\Delta w_j$, dado que, Si la neurona o perceptrón predice correctamente la etiqueta de clase, los pesos no cambian.**
...
![](/home/juan-c/snap/marktext/9/.config/marktext/images/2022-11-10-21-33-12-image.png)
