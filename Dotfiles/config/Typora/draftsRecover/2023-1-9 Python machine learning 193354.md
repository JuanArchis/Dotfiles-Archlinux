...
### Descenso de gradiente (Cuando se toquen las redes con múltiples capas se ahondará en este)
...
class Adaline(object):
    ...
    def fit(self, X, y): # Define la función de entrenamiento
        ...
        for i in range(self.n_iter): # Comienza la parte iterativa del algoritmo
            ...
            cost = (errors**2).sum() / 2.0 # Cuantifica la función de Error o costo (ECM); erros^2.sum(cost): suma los errores al cuadrado
...
Se grafica la cantida de iteraciones vs ECM (Error)
